...
const makeFiatCollateralTestSuite = (
  ...
  const deployCollateral = async (opts: MAFiatCollateralOpts = {}): Promise<TestICollateral> => {
    ...
    const NumSelfReferentialCollateralFactory: ContractFactory = await ethers.getContractFactory(
      'NumSelfReferentialCollateral'
    )
    const collateral = <TestICollateral>await NumSelfReferentialCollateralFactory.deploy(
      {
        erc20: opts.erc20,
...
makeFiatCollateralTestSuite(
  'NumSelfReferentialCollateral',
  makeOpts(tokens.nuARS!, chainlinkFeeds.nuARS!, ETH_ORACLE_TIMEOUT, ETH_ORACLE_ERROR)
)
